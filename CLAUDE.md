...
|---|---|---|---|
| **SIMBAD** (CDS) | Canonical object metadata — proper names, Bayer/Flamsteed designations, HD/HIP ids, spectral + object type | CDS Strasbourg | **Phase 3b — BAKED at ingest, not live.** Also used by the DSO ingest. |
| **NASA Exoplanet Archive** | Confirmed exoplanets + host stars, cross-matched via `gaia_dr3_id` | **NASA/IPAC** | **Phase 3b — BAKED at ingest, not live** |
...
### `GET /api/v1/objects/{source_id}`
Star enrichment lookup, keyed by Gaia DR3 `source_id` (string). Served from a **baked in-memory map** — the request path makes **zero external calls**.

- Known star → `200 {"found": true, "enrichment": {...}}`
- Unknown source_id → `200 {"found": false, "enrichment": null}` (not an error — the tooltip degrades to Gaia-only)
- Catalog file missing → `503`

`enrichment` carries `proper_name`, `designation`, `catalog_ids`, `spectral_type`, `object_type`, `planets` (present only for confirmed hosts, `{count, names}`), and `sources`. Every field except `source_id` may be null — coverage is honest-partial.
...
### Phase 3b — BUILT 2026-08-17 on `feat/phase-3b-enrichment` (awaiting QA + merge)

All 9 plan tasks executed TDD. Backend **128** default tests + **3** network / frontend **193**, lint clean. Bake: 12,119 naked-eye stars (G ≤ 6.5) → **12,037 resolved (99.3%)**, **185** exoplanet hosts, `star_enrichment.json` 3.26 MB (committed, not gitignored — `server/data/*.json` was never ignored, so the plan's `.gitignore` step was a no-op; the `!` line was added for consistency only).

**Five defects found and fixed during execution** (details in guardrail #25 and the commit messages):
1. **SIMBAD TAP silently truncates at 10,000 rows** — the biggest one. The plan's chunk_size 400 with a full identifier join lost 64 stars per chunk, and the resulting 90% resolution looked like normal coverage. Caught only by an end-to-end check (Alkaid resolved in isolation but was absent from the bake). Fixed → 99.3%.
2. `pscomppars.gaia_id` does not exist; the column is `gaia_dr3_id` (and `gaia_dr2_id` must not be substituted).
3. `otype_txt` bakes cryptic codes (`PM*`) instead of readable text — needs the `otypedef` join.
4. SIMBAD's fixed-width catalog padding (`HD   3712`) would have rendered verbatim in the tooltip; ordering was also non-deterministic.
5. The plan's `getByText("VEGA")` assertion could never pass — the uppercase is a CSS transform and RTL matches `textContent`.

> **⚠ KNOWN GAP surfaced by this phase — the Gaia subset has no bright stars.** `gaia_dr3_g9.parquet` bottoms out at **G = 1.73**; there are zero stars brighter than that. So Sirius, Vega, Arcturus, Capella, Rigel, Procyon, Betelgeuse, Altair, Aldebaran, Antares, Spica, Pollux, Fomalhaut, Deneb and Regulus are **absent from the chart entirely** — the enrichment tooltip can never show them, because they are not rendered in the first place. This is Gaia DR3's own bright-end incompleteness (it saturates below G ≈ 3), not an ingest filter bug: `ingest_gaia.py` queries `phot_g_mean_mag < 9.0` with no lower bound. Only 226 of the 12,037 enriched stars have proper names, because most IAU-named stars are exactly the bright ones Gaia is missing. **Fixing this needs a bright-star supplement from another catalog (Hipparcos/Yale BSC, already partly vendored for constellations) merged into the render set.** Not in Phase 3b's scope — Andrew's call.

**NEXT:** Andrew QAs Phase 3b live, then merge. Then decide on the bright-star gap above, then Phase 5 (deploy).
...
24. **Observation-time parsing goes through `app/services/time_utils.parse_utc_time`.** It is the single place a `datetime` query param becomes an Astropy `Time`: accepts Z-suffix, `±HH:MM` offsets, and naive-UTC strings; raises `InvalidObservationTimeError`, which every router maps to a 422. Don't reintroduce ad-hoc `.replace("Z", "")` parsing in services.
25. **Star enrichment is fully BAKED at ingest — the request path makes ZERO external calls.** `scripts/ingest_star_enrichment.py` resolves the naked-eye set (G ≤ 6.5) through SIMBAD and cross-matches the NASA Exoplanet Archive once, writing `server/data/star_enrichment.json` (committed, ~3 MB). `GET /api/v1/objects/{source_id}` is an in-memory dict lookup. **Never reintroduce live SIMBAD/NASA queries on the request path** — that was the whole architectural point (instant tooltips, offline-capable, no third-party rate limit on a click). Coverage is **honest-partial**: 12,037 of 12,119 stars resolved (99.3%); unresolved stars have no entry and the tooltip degrades to Gaia-only. **Never fake a name.** Ingest gotchas, all found the hard way — (a) **SIMBAD's TAP endpoint truncates every result at 10,000 rows, silently.** The identifier join must therefore stay narrow: it LEFT JOINs only the `NAME`/`*`/`HD`/`HIP` prefixes (~4 rows per star instead of ~30). At the plan's original chunk_size 400 with an unfiltered join, each chunk asked for ~12,000 rows, got 10,000, and lost 64 stars per chunk with no error — resolution looked like a plausible "90%". `fetch_simbad` now raises if a chunk ever comes back at the cap. **Never widen that join without lowering chunk_size.** (b) the Exoplanet Archive cross-match column is `gaia_dr3_id`, **NOT `gaia_id`** (which does not exist and fails with ORA-00904); a `gaia_dr2_id` column also exists and must NOT be used, since DR2/DR3 source_ids are not interchangeable; (c) `basic.otype_txt` returns cryptic short codes (`PM*`, `a2*`), so the query LEFT JOINs `otypedef` for `otype_longname`; (d) SIMBAD right-aligns catalog numbers to fixed width (`HD   3712`) — `build_name_fields` collapses that padding and sorts HD before HIP so the tooltip subtitle is stable across stars; (e) masked astropy cells stringify to `"--"` and must collapse to None.
